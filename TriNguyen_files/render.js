...
(function () {
  ...
  function extractObjectives(md) {
    ...
    const start = lines.findIndex(line => /^\s*\*\*Learning Objectives:?\*\*\s*$/.test(line));
    ...
    const objectives = lines.slice(start + 1, end).join('\n').trim();
  ...
  function extractIntro(md) {
    md = md.replace(/^\uFEFF?\s*#\s+Chapter[^\n]*\n/, '');
    md = md.replace(/^\s*\*NASA Cosmic[^\n]*\*\s*\n/m, '');
    md = md.replace(/^\s*\*Based on the lecture[^\n]*\*\s*\n/m, '');
